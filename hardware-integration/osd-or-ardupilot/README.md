...
[![../\_images/osd.jpg](https://ardupilot.org/copter/_images/osd.jpg)](https://ardupilot.org/copter/_images/osd.jpg)
...
Several options can be set using [OSD\_OPTIONS](https://ardupilot.org/copter/docs/parameters.html#osd-options) (bitmask selection):
...
Several font sets are included and can be changed during runtime (and those who can build the software can customize or add additional font sets….see libraries/AP\_OSD/fonts/README.md)

* [OSD\_FONT](https://ardupilot.org/copter/docs/parameters.html#osd-font):
...
In firmware 4.1 and above, onboard font options might be limited on 1 MB boards to save memory for other features. However, font upload from SD card is supported. This allows the user to choose from up to 10 font files (named “font0.bin” - “font9.bin”) placed in the root directory of your SD card. Additionally, this allows the user to easily add individualized and/or customized language charsets for the onboard OSD without the need to compile individual firmware. Runtime font changing using the parameter [OSD\_FONT](https://ardupilot.org/copter/docs/parameters.html#osd-font) is unaffected.

MAVftp might be a convenient option to upload files to your SD card without having to physically remove it from your autopilot. Font files, font editor, and utilities to convert to a .bin file from .mcm are [here](https://github.com/ArduPilot/ardupilot/tree/master/libraries/AP_OSD/fonts). If downloading the editor or a font’s .bin file from this site, be sure to use the ```Download` button, rather than selecting ``Raw```, and then right click saving, as you would the .mcm or .py files.

To enable the use of external MSP OSDs or DisplayPort systems see:

* [Minim OSD Quick Installation Guide](https://ardupilot.org/copter/docs/common-minim-osd-quick-installation-guide.html#common-minim-osd-quick-installation-guide)
* [MSP OSD](https://ardupilot.org/copter/docs/common-msp-osd-overview-4.2.html#common-msp-osd-overview-4-2)
* [OSD based parameter menu](https://ardupilot.org/copter/docs/common-paramosd.html#common-paramosd)

### Second OSD[¶](https://ardupilot.org/copter/docs/common-osd-overview.html#second-osd)
...
* [OSD\_TYPE](https://ardupilot.org/copter/docs/parameters.html#osd-type) = 1 to enable the analog system, with a long range video transmitter connected to the Video output pin of the autopilot
* [OSD\_TYPE2](https://ardupilot.org/copter/docs/parameters.html#osd-type2) = 5 to enable the second DisplayPort backend
...
There are different switch-method options to meet individual RC systems switch layout requirements. These can be set by parameter: [OSD\_SW\_METHOD](https://ardupilot.org/copter/docs/parameters.html#osd-sw-method). The options are:

* 0 = switches to next screen if the set RC channel’s ([OSD\_CHAN](https://ardupilot.org/copter/docs/parameters.html#osd-chan)) value is changed
...
ArduPilot calculates an sensor-less airspeed estimate that is used if no sensor is present or fails. [ARSPD\_TYPE](https://ardupilot.org/copter/docs/parameters.html#arspd-type) must be set to zero in order to display this value as the airspeed item, if no sensor is present.
...
* [OSD\_W\_BATVOLT](https://ardupilot.org/copter/docs/parameters.html#osd-w-batvolt) - Blinks battery voltage panel if less than this value. 0-100V with up tenth volt precision
* [OSD\_W\_RSSI](https://ardupilot.org/copter/docs/parameters.html#osd-w-rssi) - Blinks rssi panel if less than this integet percentage value. 0-99%
* [OSD\_W\_NSAT](https://ardupilot.org/copter/docs/parameters.html#osd-w-nsat) - Blinks #Sat panel if less than this number. 0-30

### Testing OSD with SITL

OSD functionality can be tested without autopilot- or video-hardware using a [Software In The Loop (SITL) simulator](https://ardupilot.org/dev/docs/sitl-simulator-software-in-the-loop.html#sitl-simulator-software-in-the-loop) setup. Follow the SITL-Instructions to setup a simulation environment. Run the simulator on current source code.

A graphical OSD simulation in a separate window can be opened by adding the following option to sim\_vehicle.py:
...
| BAT\_VOLT  | Primary Battery Voltage                                                                                       |
...
| VSPEED     | Climb Rate                                                                                                    |
| ESCTEMP    | ESC Temperature (if ESC Telem available, ESC used is selected by OSDx\_ESC\_IDX)                              |
| ESCRPM     | ESC RPM (if ESC Telem available, ESC used is selected by OSDx\_ESC\_IDX)                                      |
| ESCAMPS    | ESC Current (if ESC Telem available, ESC used is selected by OSDx\_ESC\_IDX)                                  |
| GPSLAT     | GPS Latitude                                                                                                  |
...
| BAT2\_VLT  | Second Battery Voltage                                                                                        |
...
| VTX\_PWR   | Video TX power setting                                                                                        |
| TER\_HGT   | Altitude above Terrain                                                                                        |
...
| LINK\_Q    | RC Link Quality                                                                                               |
| RPM        | Rpm reported from RPM sensor                                                                                  |
| FENCE      | Fence(s) enabled/disabled status                                                                              |
| RC\_PWR    | CRSF RX link transmit power (if “OSD panels with extended link stats data” feature enabled in a custom build) |
| RSSIDBM    | CRSF RSSI in dBm (if “OSD panels with extended link stats data” feature enabled in a custom build)            |
| RC\_SNR    | CRSF RC SNR (if “OSD panels with extended link stats data” feature enabled in a custom build)                 |
| RC\_ANT    | CRSF active antenna (if “OSD panels with extended link stats data” feature enabled in a custom build)         |
| RC\_LQ     | CRSF Link Quality (if “OSD panels with extended link stats data” feature enabled in a custom build)           |
